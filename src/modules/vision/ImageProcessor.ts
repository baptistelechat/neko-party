...
export class ImageProcessor {
  ...
  public processCrop(
    ...
    let inputForOpenCV: HTMLImageElement | HTMLCanvasElement | string =
      source as unknown;

    // Handle HTMLVideoElement manually: draw to temporary canvas first
    // cv.imread(video) can be flaky or require specific ID strings.
    if (source instanceof HTMLVideoElement) {
      ...
      inputForOpenCV = tempCanvas;
    }

    try {
      // 1. Read source into Mat
      const src = cv.imread(inputForOpenCV);
